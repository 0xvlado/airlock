...
- **Aikido Safe Chain integration** — `install_binary` plugin phase with prefix mode wraps the install command through aikido's `safe-chain` malware-scanning proxy
- **Plugin auto-detection in setup.sh** — automatically creates plugins config if `safe-chain` binary is found on PATH
